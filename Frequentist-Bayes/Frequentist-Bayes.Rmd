...
library(knitr)
library(crayon)
knitr::opts_chunk$set(fig.width=12, fig.height=8, fig.path='Figs/',include=FALSE,echo=FALSE, warning=FALSE, message=FALSE,showAnswers=TRUE,results= 'asis')
```
...
$P(1) = P(A \\cap +) = P(A) \\times P(+|A) = 0.01 \\times 0.8 = 0.008$, 
...
cat(
"<b>A</b>",qcount,":\n 
$P(2) = P(A \\cap -) = P(A) \\times P(-|A) = 0.01 \\times 0.2 = 0.002$ 
  
$P(3) = P(\\bar{A} \\cap +) = P(\\bar{A}) \\times P(+|\\bar{A}) = 0.99 \\times 0.96 = 0.9504$  
  
$P(4) = P(\\bar{A} \\cap -) = P(\\bar{A}) \\times P(-|\\bar{A}) = 0.99 \\times 0.04 = 0.0396$ 
",sep="")
```
...
qcount=qcount+1
cat(
"<b>Q</b>",qcount,": Check that $P(1) + P(2) + P(3) + P(4) = 1$.  
...
(Hint: this is the sum of two of the outcomes you calculate in Q
",qcount-2,")",sep="")
```

```{r}
cat(
"<b>A</b>",qcount,": $P(+) = P(1) + P(3)= 0.008 + 0.9504 = 0.9584$
",sep="")
```
...
(Hint: you can find this using the two values that you used in Q
",qcount-3,").",sep="")
```

```{r}
cat(
"<b>A</b>",qcount,": $P(A|+) = \\dfrac{P(1)}{P(1)+P(3)} = \\dfrac{0.008}{0.008 + 0.9504}= 0.0084$
",sep="")
```
...
```{r}
cat(
"<b>A</b>",qcount,": $$P(A|+)=\\dfrac{P(+|A)\\cdot P(A)}{P(+)}=\\dfrac{0.008\\times 0.01}{0.9504}=0.0084$$
",sep="")
```
